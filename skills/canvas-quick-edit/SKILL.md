...
description: "Run a user-described Quick Edit on a selected Museboard image and collect the result back onto the canvas."
---

# Museboard Quick Edit

Use this skill when the user invokes Quick Edit from Museboard or asks to perform an open-ended edit on a selected canvas image.
...
8. Generate exactly one revised clean bitmap. Save the final selected output as a PNG under the job output directory provided by Museboard.
9. Museboard will collect the output, remove the chroma-key background when needed, and replace the running placeholder to the right of the source image. It will preserve the original image and its annotation objects.
