...
description: "Remove the background from a selected Museboard image and collect a transparent PNG result back onto the canvas."
---

# Museboard Remove BG

Use this skill when the user invokes Remove BG from Museboard or asks to isolate the foreground subject of a selected canvas image.
...
9. Save the generated chroma-key PNG under the Museboard job output directory.
10. Museboard will remove the chroma key locally using its bundled chroma-key helper with soft matte and despill, verify the RGBA alpha PNG, collect it, and place it in a row to the right of the source image.
