---
name: canvas-expand
description: "Outpaint a selected Museboard image beyond its current frame and collect the expanded image back onto the canvas."
---

# Museboard Expand

Use this skill when the user invokes Expand from Museboard or asks to extend a selected canvas image beyond its current edges.

## Behavior

1. Museboard first creates a padded PNG input using the selected scale, target ratio, and the user's final source-image position inside the preview frame. Treat the pasted original content as locked source content and the padded surroundings as the outpaint region.
...
7. Save the final expanded image as a PNG under the Museboard job output directory.
8. Museboard will collect the output and place it in a row to the right of the source image.
